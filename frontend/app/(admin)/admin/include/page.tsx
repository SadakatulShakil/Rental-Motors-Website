...
import { 
  ...
  Edit2, X, CheckCircle2, AlertCircle, Check
} from "lucide-react"
...
export default function AdminIncludedPage() {
  ...
  const notify = (msg: string) => {
    alert(`✅ ${msg}\n\nNote: Please click the main 'SAVE CHANGES' button at the top to sync all site data.`);
  }
    ...
    <div className="p-8 max-w-6xl mx-auto bg-white rounded-[2.5rem] shadow-sm border border-slate-100 space-y-12 mb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="space-y-6">
          <SectionHeader icon={<Layout size={18}/>} title="Banner & Titles" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          <div className={`p-8 rounded-[2.5rem] border-2 transition-all space-y-4 ${editFeatId ? 'bg-blue-50 border-blue-100' : 'bg-slate-950 border-slate-900 shadow-xl'}`}>
            <div className="grid grid-cols-5 gap-2 pb-4">
                {ICON_OPTIONS.map(icon => (
                    <button key={icon} onClick={() => setNewFeat({...newFeat, icon_name: icon})} className={`p-2 rounded-lg text-[9px] font-black transition-all ${newFeat.icon_name === icon ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-500 hover:text-white'}`}>
                        {icon.replace("Fa", "")}
                    </button>
                ))}
            </div>
            <input placeholder="Feature Title" className="w-full bg-transparent border-b border-slate-800 text-white p-2 font-bold outline-none focus:border-blue-500" value={newFeat.title} onChange={e => setNewFeat({...newFeat, title: e.target.value})} />
            <input placeholder="Subtitle" className="w-full bg-transparent border-b border-slate-800 text-white p-2 font-bold outline-none focus:border-blue-500" value={newFeat.subtitle} onChange={e => setNewFeat({...newFeat, subtitle: e.target.value})} />
            <div className="flex gap-2">
                <button onClick={handleSaveFeature} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black italic text-[10px] uppercase">
                    {editFeatId ? "Update Feature" : "Add Feature"}
                </button>
                {editFeatId && <button onClick={() => {setEditFeatId(null); setNewFeat({icon_name:"FaMotorcycle", title:"", subtitle:""})}} className="p-3 bg-white/10 text-white rounded-xl"><X size={16}/></button>}
            </div>
          </div>

          <div className="space-y-2">
            {features.map((f: any) => (
              <div key={f.id} className="p-4 bg-white border border-slate-100 rounded-2xl flex justify-between items-center group">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-[10px] font-black text-blue-600 uppercase tracking-tighter">{f.icon_name.slice(2,5)}</div>
                    <div>
                        <p className="font-black text-xs uppercase italic">{f.title}</p>
        ...
        <div className="space-y-6">
          ...
          <div className={`p-8 rounded-[2.5rem] border-2 space-y-4 ${editPolicyId ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/20'}`}>
            ...
            <textarea placeholder="Points (comma separated)" className="w-full border-b border-slate-100 p-2 font-bold outline-none focus:border-blue-600 text-sm h-16 resize-none" value={newPolicy.points} onChange={e => setNewPolicy({...newPolicy, points: e.target.value})} />
            <div className="flex gap-2">
                <button onClick={handleSavePolicy} className="flex-1 bg-slate-950 text-white py-3 rounded-xl font-black italic text-[10px] uppercase">
                    {editPolicyId ? "Update Policy" : "Create Card"}
                </button>
                {editPolicyId && <button onClick={() => {setEditPolicyId(null); setNewPolicy({title:"", points:"", color_type:"orange"})}} className="p-3 bg-slate-100 rounded-xl"><X size={16}/></button>}
            </div>
          </div>

          <div className="space-y-3">
            {policies.map((p: any) => (
              <div key={p.id} className={`p-5 rounded-[1.5rem] flex justify-between items-center text-white ${p.color_type === 'orange' ? 'bg-orange-500' : 'bg-slate-900'}`}>
                <div className="truncate pr-4">
                    <p className="font-black uppercase italic text-sm">{p.title}</p>
                    <p className="text-[8px] font-bold opacity-70 truncate uppercase">{p.points}</p>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => {setEditPolicyId(p.id); setNewPolicy({...p});}} className="p-2 bg-white/20 rounded-lg"><Edit2 size={14}/></button>
                  <button onClick={async () => { if(confirm("Delete?")) { await fetch(`${apiUrl}/admin/include/policies/${p.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); fetchData(); } }} className="p-2 bg-white/20 rounded-lg"><Trash2 size={14}/></button>
                </div>
              </div>
            ))}
          </div>
...
const AdminInput = ({ label, ...props }: any) => (
  <div className="flex flex-col gap-2">
    ...
    <input {...props} className="w-full border-b border-slate-100 py-3 font-bold outline-none focus:border-blue-600 transition-all text-slate-900 bg-transparent" />
  </div>
);