...
import { 
  ...
  Loader2, Plus, Edit2, X, Gauge, 
  Fuel, Settings, DollarSign, Calendar, Palette
} from "lucide-react"
...
export default function AdminBikesPage() {
  ...
  if (fetching) return <div className="p-10 text-center animate-pulse italic font-black">Loading Fleet...</div>
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto bg-white rounded-[2.5rem] shadow-sm border border-slate-100 space-y-12 mb-20">
      ...
      <div className="flex items-center justify-between border-b pb-6">
        <div>
          <h1 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900">Fleet Management</h1>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Inventory & Pricing Control</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => { setIsAdding(!isAdding); setEditSlug(null); setFormData(initialForm); }} 
            className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black italic flex items-center gap-2 hover:bg-blue-600 transition-all text-xs uppercase"
          >
            {isAdding ? <X size={16}/> : <Plus size={16}/>} {isAdding ? "Cancel" : "Add Vehicle"}
          </button>
        </div>
      </div>
      ...
      {!isAdding && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="space-y-6">
            ...
            <button onClick={handleSaveMeta} className="bg-blue-600 text-white px-6 py-2 rounded-xl font-black italic text-[10px] uppercase">Update Banner Text</button>
          </div>
          <div className="border-2 border-dashed border-slate-100 rounded-[2rem] p-6 bg-slate-50/50 flex flex-col items-center">
             {metaData.header_image ? <img src={metaData.header_image} className="h-40 w-full object-cover rounded-2xl mb-4 shadow-sm" /> : <div className="h-40 w-full bg-white rounded-2xl mb-4" />}
             <label className="w-full cursor-pointer">
                <div className="flex items-center justify-center gap-2 py-3 bg-white rounded-xl shadow-sm border border-slate-100 font-black italic text-[10px] uppercase">
                    <Upload size={14} /> Upload Header Image
                </div>
          ...
          <div className="overflow-hidden border border-slate-100 rounded-[2.5rem] shadow-sm">
            <table className="w-full text-left border-collapse bg-white">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Vehicle Details</th>
                  <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Pricing</th>
                  <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Specs</th>
                ...
                {bikes.map((bike) => (
                  <tr key={bike.slug} className="group border-b border-slate-50 hover:bg-slate-50/50 transition-all">
                    <td className="p-6 flex items-center gap-5">
                       <img src={bike.image} className="w-24 h-16 object-contain bg-slate-50 rounded-xl border border-slate-100 p-2 group-hover:scale-105 transition-transform" />
                       <div>
                          <p className="font-black uppercase italic text-slate-900 tracking-tight">{bike.name}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase">{bike.type} • {bike.color}</p>
                       </div>
                    </td>
                    <td className="p-6">
                       <p className="text-blue-600 font-black italic text-lg">{bike.price}</p>
                       <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Starting Daily</p>
                    </td>
                    <td className="p-6">
                       <div className="flex gap-2">
                          <span className="px-2 py-1 bg-slate-100 rounded text-[9px] font-black text-slate-500">{bike.cc}</span>
                          <span className="px-2 py-1 bg-slate-100 rounded text-[9px] font-black text-slate-500">{bike.transmission}</span>
                       </div>
                    </td>
                    <td className="p-6 text-right">
                       <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setEditSlug(bike.slug); setFormData(bike); setIsAdding(true); }} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(bike.slug)} className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm">
                            <Trash2 size={16} />
                          </button>
                       </div>
                    </td>
